            ...
            try:
                ...
                while True:
                    ...
                    if (response):
                        ...
                        self.logger.info(
                            "1--cnt:%s,send_cmd:uuid:%s,cmd:%s,ret:%s,difftime:%s,response:%s", cnt, uuid, cmd, ret, diff, ret_all)
                        # time.sleep(0.1)
                        s1 = re.compile('^(-?[1-9]|0{1}\d*)$')
                        r1 = s1.findall(ret)
                        if (len(r1) > 0):
                            self.logger.info(
                                "send_cmd:uuid:%s,cmd:%s,ret:%s,difftime:%s,response:%s", uuid, cmd, ret, diff, ret_all)
                            ret_dict = {
                                "uuid": uuid,
                                "cmd": cmd,
                                "retsult": ret,
